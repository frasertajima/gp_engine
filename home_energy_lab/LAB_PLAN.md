...
> ladder is a statistical tie at the top** (all eight proactive methods within $36/yr, top three
> within $5); the
> real effects are proactive off-peak pre-charging (~$130-165/yr) and a peak-window discharge reserve
> (~$25/yr), neither of them a forecasting result. **Phase 3's optimum is 4-6kW solar / no battery**
> (a $7/yr margin, not the $107/yr the uncredited-export version implied).
>
> **H1 changed two of the C1/C2 conclusions, and the write-ups were corrected rather than left
> standing.** Crediting export moved the GP from $1 *behind* a model-free calendar rule to ~$5
> *ahead*, and shrank the regime layer's penalty from $22/yr to $4/yr — a null, not a negative. The
...
> $515/yr, mid-pack) and now scores both rate structures as named alternatives.
> **Remaining open: only the M and L items.**
...
- **Real, current solar/battery cost anchors**: Tesla Powerwall 3 ($11,500-$16,500 installed, 13.5
  kWh), $2.55-$3.45/W solar installed, 30% federal tax credit through 2032
  (`research/03_solar_battery_economics.md`). Battery round-trip-efficiency/degradation spec
  explicitly **not yet sourced** — deferred to Phase 0, not invented.
...
ladder is a statistical tie.** All eight proactive methods sit within $36/yr and the top three
within $5 — about 1% of the bill. The two effects large enough to be real are **proactive off-peak pre-charging ($649 ->
~$490/yr, ~$160)** and **a peak-window discharge reserve (~$25/yr)**; neither is a forecasting
result. Best is GP + constant reserve at $485/yr, with the model-free calendar + same reserve at
$490 and the regime-sized version at $489. The GP's own predictive RMSE (7.04) barely improves on
...
export credit, $4/yr after it. A legitimate fourth instance of this codebase's "regime-awareness
...
over 6kW collapsed from $107/yr to $7/yr** — the honest answer is now "4-6kW of solar, no battery",
since a $7/yr gap on a $1,254/yr total is inside the model's own resolution. Battery capacity still
...
- `capacity_sizing.py` / `results_phase3.json` / `RESULTS_PHASE3.md` — **RE-RUN 2026-08-05.** 4-6kW
  solar / 0kWh battery wins on pure cost grounds ($1,254 vs $1,261/yr, effectively tied); two real
...
- `research/` — **DONE**, eight passes (08 added 2026-08-05: BC Hydro export compensation).
