"""Phase 2 rerun v3: port the SOFT (responsibility-weighted) regime
classifier to OOC scale.

phase2_rerun_more_history.py (v2) gave method 4 a fair test with 200
historical years and a fresh p_hat, but still used regime_mixture.py's HARD
stress/normal partition (12/188 years) for the OOC sigma refinement --
achieved survival landed at 96.11%, short of the 99.5% target.
phase1_soft_sweep.py then tested a SOFT classifier at small scale
(regime_mixture.fit_regime_mixture_soft: every year contributes to BOTH
components' fits, weighted by its own posterior P(systemic), no hard
cutoff) and found it closes nearly all of the remaining gap to the
oracle-cheat ceiling at every historical-years size tested (RESULTS_PHASE1.md).

This script ports that soft classifier to n=45,000: instead of splitting
losses_hist into is_stress/~is_stress subsets and running OOC refinement on
each subset separately, EVERY historical year is used for BOTH components'
OOC refinement, weighted by that year's GaussianMixture responsibility
(gp_loss_model_large.refine_sigma_scale_ooc's new `weights` parameter).
Same 200-year historical sample, same book, same oracle as v2, so the
result is directly comparable to RESULTS_PHASE2.md's v2 numbers.

Everything else (methods 1-3, the RFF-based scenario generation, the
oracle) is unchanged from phase2_rerun_more_history.py.
"""
...
OOC_BACKING = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".ooc_backing_phase2_soft")
OUTPUT_JSON = "results_phase2_soft.json"
...
def main():
    ...
    print(f"=== Phase 2 rerun v3: n=45,000, {HISTORICAL_YEARS_LARGE}-year history, "
          f"SOFT classifier ===\n", flush=True)
    ...
    print("Method 4 (GP + regime-mixture, SOFT classifier)... "
          "OOC refinement (3 evals x 2 components, all years weighted):", flush=True)
    ...
    resp = gmm.predict_proba(total_log_hist.reshape(-1, 1))[:, systemic_component]
    ...
    print(f"  mean responsibility={resp.mean():.4f} -- every year contributes to BOTH "
          f"components' fits, no hard stress/normal split", flush=True)

    log_ratio = np.log(losses_hist / V[None, :])
    w_normal = 1.0 - resp
    w_systemic = resp
    mu_n = np.average(log_ratio, axis=0, weights=w_normal)
    R_n = log_ratio - mu_n[None, :]
    refined_n = gpl_large.refine_sigma_scale_ooc(
        ...
        ram_budget_gb=OOC_RAM_BUDGET_GB, weights=w_normal)
    print(f"  normal component refined: sigma_f2={refined_n['sigma_f2']:.4f} "
          f"sigma_n2={refined_n['sigma_n2']:.4f}", flush=True)

    mu_s = np.average(log_ratio, axis=0, weights=w_systemic)
    R_s = log_ratio - mu_s[None, :]
    refined_s = gpl_large.refine_sigma_scale_ooc(
        ...
        ram_budget_gb=OOC_RAM_BUDGET_GB, weights=w_systemic)
    ...
    output = dict(oracle_true_capital=oracle_true_capital, oracle_years=ORACLE_YEARS_LARGE,
                  ...
                  params=dict(n_properties=N_PROPERTIES_LARGE,
                              ...
                              method4_p_hat_refit=dict(value=p_hat, raw=p_hat_raw,
                                                        mean_responsibility=float(resp.mean()),
