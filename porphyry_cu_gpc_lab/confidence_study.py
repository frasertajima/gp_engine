#!/usr/bin/env python3
"""Multi-seed robustness study for the porphyry Cu ore/waste confident-wrong
question, same methodology as `../mining_gpc_lab/confidence_study.py` (and,
before that, `place_gpc_lab`/`mnist_gpc_lab`'s M2) -- a single seed's test
split has only ~77 ore points (~5% of ~1,526), so single-seed confident-
wrong counts are not worth reading a percentage off of directly.
...
selected on average precision -- same reasoning as every other lab in this
family: re-running a 5-point validation grid per seed is the expensive
part, not the fit itself.

Usage: python3 confidence_study.py --n-seeds 200 [--ell 100.27]
...
def main():
    ...
    ap.add_argument("--ell", type=float, default=100.27)
